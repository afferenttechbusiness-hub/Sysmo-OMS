...
export function useTasks() {
  ...
  const load = useCallback(async () => {
    let query = supabase.from('tasks').select('*, project:projects(*)').order('created_at', { ascending: false });
...
export function useTeams() {
  ...
  const load = useCallback(async () => {
    const { data } = await supabase.from('team_groups').select('*, project:projects(*)').order('created_at', { ascending: false });
...
export function useNotices() {
  ...
  const load = useCallback(async () => {
    const { data } = await supabase.from('notices').select('*, author:profiles(*)').order('created_at', { ascending: false });
